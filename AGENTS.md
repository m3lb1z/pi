...
- Omit all verification tests during code review; only analyze and check for syntax problems or errors.
- Never run `npm run build` or `npm test` unless requested by the user.
- Do not run test suites (vitest, node:test, test.sh) during code reviews or changes unless explicitly asked by the user.
