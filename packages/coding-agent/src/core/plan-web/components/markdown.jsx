...
import { Code2, Maximize2, MessageSquareText, Minimize2, RotateCcw, SquarePen, Workflow, X, ZoomIn, ZoomOut } from "lucide-react";
...
function MermaidBlock({ source }) {
	...
	return (
		<section className={`mermaid-card${expanded ? " expanded" : ""}`} aria-label="Diagrama Mermaid">
			...
			{showSource ? (
				<pre className="mermaid-source"><code className="hljs language-mermaid">{source}</code></pre>
			) : error ? (
				...
				>
					<div className="diagram-canvas" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})` }} dangerouslySetInnerHTML={{ __html: svg }} />
				</div>
			)}
		</section>
	);
...
export function Markdown({ annotations, content, onSelection, selection }) {
	...
	useEffect(() => {
		...
		if (globalThis.CSS?.highlights && globalThis.Highlight) {
			...
			const selectionRange = selection ? rangeFromOffsets(container.current, selection.start, selection.end) : null;
	...
	return <div ref={container} className="markdown" onMouseUp={captureSelection} onKeyUp={captureSelection} dangerouslySetInnerHTML={{ __html: html }} />;
}

export function SelectionToolbar({ selection, onCancel, onComment, onEdit }) {
	...
	return (
		<div className={`selection-toolbar${position.below ? " below" : ""}`} style={{ left: position.left, top: position.top }} role="toolbar" aria-label="Acciones para la selección">
			<IconButton label="Comentar selección" onClick={onComment}><MessageSquareText /></IconButton>
			<IconButton label="Editar Markdown" onClick={onEdit}><SquarePen /></IconButton>
			<span className="selection-toolbar-divider" />
			<IconButton label="Cancelar selección" onClick={onCancel}><X /></IconButton>
		</div>
	);
}
