...
describe("plan web server", () => {
	it("serves the page without exposing plan contents or the approval token", async () => {
		...
		expect(app).toContain("Pi Planning");
		expect(app).toContain("Aprobar");
		expect(app).not.toContain("Nuevo plan");
		expect(app).toContain("Descartar");
		expect(app).toContain("Descartar plan");
		expect(app).toContain("alertdialog");
		expect(app).toContain("Refinar");
		expect(app).toContain("Acciones para la selecci");
		expect(app).toContain("language-mermaid");
		expect(app).toContain("YOUR PLAN WAS NOT APPROVED");
		expect(style).toContain("::highlight(plan-selection)");
